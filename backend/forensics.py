...
try:
    forensic_ai_path = os.path.join(os.path.dirname(__file__), 'forensic_ai')
    if os.path.exists(forensic_ai_path):
        sys.path.insert(0, forensic_ai_path)
    ...
    if _ai_engine is None and HAS_FORENSIC_AI:
        try:
            checkpoint_dir = os.environ.get(
                'FORENSIC_CHECKPOINT_DIR',
                os.path.join(os.path.dirname(__file__), 'forensic_ai', 'outputs', 'checkpoints')
            )
            os.makedirs(checkpoint_dir, exist_ok=True)
            _ai_engine = ForensicEngine(checkpoint_dir=checkpoint_dir)
            _ai_engine.warmup()
        except Exception as e:
            logger.warning("Failed to warm up Forensic AI Engine: %s", e)
    return _ai_engine
