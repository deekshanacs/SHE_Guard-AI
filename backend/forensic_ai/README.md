...
The inference engine will automatically load trained weights from `outputs/checkpoints` by default, or from the `FORENSIC_CHECKPOINT_DIR` environment variable when it is set in production.
...
This matches the Render configuration used by the backend service, so dropping trained checkpoints into the mounted checkpoint directory is enough to activate them.
