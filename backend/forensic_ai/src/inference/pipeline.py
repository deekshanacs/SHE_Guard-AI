...
from ..models.model2_vit import ForensicViT
...
FUSION_INPUT_DIM = 72  # 8 base scores + 56 pairwise features + 8 summary stats
...
class ModelRegistry:
    ...
    def _checkpoint_candidates(self, name: str) -> List[Path]:
        candidates = [
            self.checkpoint_dir / f"{name}_best.pth",
            self.checkpoint_dir / f"{name}.pth",
            self.checkpoint_dir / f"{name}.pt",
            self.checkpoint_dir / f"model_{name}_best.pth",
            self.checkpoint_dir / f"best_{name}.pth",
        ]
        if self.checkpoint_dir.exists():
            for pattern in (f"**/{name}_best.pth", f"**/{name}.pth", f"**/{name}.pt"):
                for path in self.checkpoint_dir.glob(pattern):
                    if path not in candidates:
                        candidates.append(path)
        return candidates

    def _load_checkpoint_state(self, model: torch.nn.Module, checkpoint_path: Path) -> torch.nn.Module:
    ...
    def _load_checkpoint(self, model: torch.nn.Module, name: str) -> torch.nn.Module:
        for ckpt_path in self._checkpoint_candidates(name):
            if ckpt_path.exists():
                self._load_checkpoint_state(model, ckpt_path)
                logger.info(f"Loaded {name} from {ckpt_path}")
                model.to(DEVICE).eval()
                return model

        logger.warning(
            "No checkpoint for %s under %s — using pretrained backbone defaults",
            name,
            self.checkpoint_dir,
        )
        model.to(DEVICE).eval()
    ...
    def _build(self, name: str) -> torch.nn.Module:
        if name == "rgb":
            ...
            return ForensicViT(backbone="vit_large", num_classes=1)
        elif name == "frequency":
    ...
    def get_fusion(self) -> Optional[Any]:
        if self._fusion is None:
            fusion_candidates = [
                self.checkpoint_dir / "fusion_best.pth",
                self.checkpoint_dir / "fusion.pth",
                self.checkpoint_dir / "fusion.pt",
                self.checkpoint_dir / "meta_best.pth",
            ]
            if self.checkpoint_dir.exists():
                for pattern in ("**/fusion_best.pth", "**/fusion.pth", "**/fusion.pt", "**/meta_best.pth"):
                    for path in self.checkpoint_dir.glob(pattern):
                        if path not in fusion_candidates:
                            fusion_candidates.append(path)

            for fusion_path in fusion_candidates:
                if fusion_path.exists():
                    state = torch.load(fusion_path, map_location=DEVICE)
                    model = NeuralMetaClassifier(input_dim=FUSION_INPUT_DIM, hidden_dims=[256, 128, 64])
                    model.load_state_dict(state.get("state_dict") or state.get("model_state_dict") or state)
                    model.to(DEVICE).eval()
                    self._fusion = model
                    logger.info(f"Loaded fusion model from {fusion_path}")
                    break
            else:
                logger.warning("No fusion checkpoint found under %s — using weighted average", self.checkpoint_dir)
        return self._fusion
...
def run_rgb_model(model: torch.nn.Module, inputs: Dict) -> Dict[str, float]:
    ...
    if isinstance(out, dict):
        prob = out.get("prob")
        logit = out.get("logit")
        manip_logit = out.get("manip_logit")
    else:
        prob, _, manip_logit = (out + (None,) * 3)[:3]
        logit = None
    if prob is not None:
        score = torch.as_tensor(prob).float().squeeze().item()
    elif logit is not None:
        score = torch.sigmoid(torch.as_tensor(logit).float()).squeeze().item()
    else:
        score = 0.5
    manip_source = manip_logit if manip_logit is not None else torch.zeros(1, 8)
    manip_probs = torch.softmax(torch.as_tensor(manip_source).float(), dim=-1).squeeze().tolist()
...
def run_vit_model(model: torch.nn.Module, inputs: Dict) -> Dict[str, float]:
    ...
    if isinstance(out, dict):
        prob = out.get("prob")
        logit = out.get("logit")
        source_logit = out.get("source_logit")
    else:
        prob, _, source_logit = (out + (None,) * 3)[:3]
        logit = None
    if prob is not None:
        score = torch.as_tensor(prob).float().squeeze().item()
    elif logit is not None:
        ...
        score = 0.5
    source_source = source_logit if source_logit is not None else torch.zeros(1, 4)
    source_probs = torch.softmax(torch.as_tensor(source_source).float(), dim=-1).squeeze().tolist()
...
def run_frequency_model(model: torch.nn.Module, inputs: Dict) -> Dict[str, float]:
    ...
    if isinstance(out, dict):
        ...
        prob = out[0] if len(out) > 0 else None
        logit = None
    if prob is not None:
        score = torch.as_tensor(prob).float().squeeze().item()
    elif logit is not None:
...
def run_noise_model(model: torch.nn.Module, inputs: Dict) -> Dict[str, float]:
    ...
    if isinstance(out, dict):
        prob = out.get("prob")
        logit = out.get("logit")
        noise_level = out.get("noise_level", torch.tensor(0.0))
    else:
        prob, _, noise_level = (out + (None,) * 3)[:3]
        logit = None
    if prob is not None:
        score = torch.as_tensor(prob).float().squeeze().item()
    elif logit is not None:
        ...
        score = 0.5
    noise_level = torch.as_tensor(noise_level if noise_level is not None else 0.0).float().squeeze().item()
...
def run_face_model(model: torch.nn.Module, inputs: Dict) -> Dict[str, float]:
    ...
    if isinstance(out, dict):
        prob = out.get("prob")
        logit = out.get("logit")
        forgery_logit = out.get("forgery_logit")
        identity_logit = out.get("identity_logit")
    else:
        prob, _, forgery_logit, identity_logit = (out + (None,) * 5)[:4]
        logit = None
    if prob is not None:
        score = torch.as_tensor(prob).float().squeeze().item()
    elif logit is not None:
        ...
        score = 0.5
    identity_score = torch.sigmoid(torch.as_tensor(identity_logit if identity_logit is not None else 0.0).float()).squeeze().item()
    forgery_source = forgery_logit if forgery_logit is not None else torch.zeros(1, 4)
    forgery_probs = torch.softmax(torch.as_tensor(forgery_source).float(), dim=-1).squeeze().tolist()
    return {
        "face_score": score,
...
def run_localization_model(model: torch.nn.Module, inputs: Dict) -> Dict[str, Any]:
    ...
    out = model(rgb)
    if isinstance(out, dict):
        mask_logit = out.get("mask")
        image_logit = out.get("image_logit")
        image_prob = out.get("image_prob")
    else:
        mask_logit, image_prob = (out + (None,) * 3)[:2]
        image_logit = None
    mask_prob = torch.as_tensor(mask_logit).float().squeeze().cpu().numpy() if mask_logit is not None else None
    if image_prob is not None:
        image_score = torch.as_tensor(image_prob).float().squeeze().item()
    elif image_logit is not None:
        image_score = torch.sigmoid(torch.as_tensor(image_logit).float()).squeeze().item()
    else:
        image_score = 0.5
